#!/usr/bin/python
#
# landscape-sysinfo-mini.py -- a trivial re-implementation of the 
# sysinfo printout shown on debian at boot time. No twisted, no reactor, just /proc & utmp
#
# (C) 2014 jw@owncloud.com
#
# inspired by ubuntu 14.10 /etc/update-motd.d/50-landscape-sysinfo
# Requires: python-utmp 
# for counting users.
#
# 2014-09-07 V1.0 jw, ad hoc writeup, feature-complete. Probably buggy?
# 2014-10-08 V1.1 jw, survive without swap
# 2014-10-13 V1.2 jw, survive without network
# 2020-04-10 V1.3 ht, converted code from python2 to python3

# Modified by Luc Didry in 2016
# Modified by Hunter T. in 2020
# Get the original version at https://github.com/jnweiger/landscape-sysinfo-mini

from __future__ import division
from __future__ import absolute_import
import sys,os,time,posix,glob,utmp
from UTMPCONST import *
from io import open

_version_ = u'1.3'

def utmp_count():
  u = utmp.UtmpRecord()
  users = 0
  for i in u:
    if i.ut_type == utmp.USER_PROCESS: users += 1
  return users

def proc_meminfo():
  items = {}
  for l in open(u'/proc/meminfo').readlines():
    a = l.split()
    items[a[0]] = int(a[1])
  # print items['MemTotal:'], items['MemFree:'], items['SwapTotal:'], items['SwapFree:']
  return items

def proc_mount():
  items = {}
  for m in open(u'/proc/mounts').readlines():
    a = m.split()
    if a[0].find(u'/dev/') is 0:
      statfs = os.statvfs(a[1])
      perc = 100-100.*statfs.f_bavail/statfs.f_blocks
      gb = statfs.f_bsize*statfs.f_blocks/1024./1024/1024
      items[a[1]] = u"%.1f%% of %.2fGB" % (perc, gb)
  return items

loadav = float(open(u"/proc/loadavg").read().split()[1])
processes = len(glob.glob(u'/proc/[0-9]*'))
statfs = proc_mount()
users = utmp_count()
meminfo = proc_meminfo()
memperc = u"%d%%" % (100-100.*meminfo[u'MemAvailable:']/(meminfo[u'MemTotal:'] or 1))
swapperc = u"%d%%" % (100-100.*meminfo[u'SwapFree:']/(meminfo[u'SwapTotal:'] or 1))

if meminfo[u'SwapTotal:'] == 0: swapperc = u'---'

print u"  System information as of %s\n" % time.asctime()
print u"  System load:  %-5.2f                Processes:           %d" % (loadav, processes)
print u"  Memory usage: %-4s                 Users logged in:     %d" % (memperc, users)
print u"  Swap usage:   %s" % (swapperc)

print u"  Disk Usage:"
for k in sorted(statfs.keys()):
  print u"    Usage of %-24s: %-20s" % (k, statfs[k])

if users > 0:
    a = utmp.UtmpRecord()

    print u"\n  Logged in users:"

    for b in a: # example of using an iterator
        if b.ut_type == USER_PROCESS:
            print u"  \033[1;31m%-10s\033[m from %-25s at %-20s" % \
            (b.ut_user, b.ut_host, time.ctime(b.ut_tv[0]))
    a.endutent()

sys.exit(0)
